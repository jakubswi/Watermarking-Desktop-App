...
def browseFiles():
    ...
    filename = filedialog.askopenfilename(initialdir="/",title="Select a File",filetypes=[("Image File",'.jpg .png .jpeg')])
    label_file_explorer.configure(text="File Opened: " + filename)
    image = Image.open(filename)
    image=image.copy()
    width, height = image.size

def browseForWatermark():
    global watermark_width, watermark_height, watermark_image
    filename_watermark = filedialog.askopenfilename(initialdir="/", title="Select a File", filetypes=[("Image File",'.jpg .png .jpeg')])
...
def add_image_watermark():
    ...
    size=(int(watermark_width*(float(slider_size.get())/100)),int(watermark_height*(float(slider_size.get())/100)))
...
button_explore = Button(window, text = "Browse Files for image",command = browseFiles)
...
button_explore_watermark = Button(window, text = "Browse Files for watermark",command = browseForWatermark)
